...
export default function Gallery() {
  ...
  const handleDeleteGallery = () => {
    const response = prompt(
      "Are you sure you want to delete your gallery? If so, type 'y' "
    );
    if (response !== "y") {
      return;
    }
      ...
      {gallery ? (
        <>
          ...
          <h3 style={{ padding: "10px", color: "red" }}>
            By:{" "}
            <Link to={`/authors/${gallery?.user?.id}`} style={{ color: "red" }}>
          ...
          {activeUser && activeUser.id === gallery.user_id ? (
            <Button variant="outline-secondary" onClick={handleDeleteGallery}>
              Delete gallery
            </Button>
          ) : (
            <></>
              ...
              {gallery.comments.map((comment) => (
                <li key={comment.id} id={`comment${comment.id}`}>
                  ...
                  {activeUser && activeUser.id === comment.user.id ? (
                    <Button variant="outline-secondary" onClick={() => handleDeleteComment(comment.id)}>
                      Delete Comment
                    </Button>
                  ) : (
                    <></>
